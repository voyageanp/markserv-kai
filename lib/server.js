...
const dirToHtml = (filePath, rootDir) => {
...
const buildRecentEntries = (rootDir, currentFilePath, limit = 10) => {
	const excludeDirs = new Set(["node_modules"]);
	const entries = [];
		...
		if (req.method === "GET" && decodedUrl === "/__markserv/ipynb-render") {
			...
			convertNotebookToMarkdown({
				...
				.then((output) => {
					...
					const recentEntries = buildRecentEntries(dir, absoluteNotebookPath, 10);
		...
		if (isMarkdown) {
			...
			getFile(filePath)
				...
				.then(({ output, editDataJson }) => {
					...
					const recentEntries = buildRecentEntries(dir, filePath, 10);
			...
			try {
				...
				const recentEntries = buildRecentEntries(dir, null, 10);

				const handlebarData = {
					dirname: path.parse(filePath).dir,
					content: dirToHtml(filePath, dir),
					title: path.basename(path.resolve(dir)),
					navTree,
					recentEntries,
					rootDirStem: path.basename(path.resolve(dir)),
