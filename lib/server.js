...
const waitForPortFree = async (port, timeoutMs = 3000) => {
...
const buildNotebookRenderHref = (relativePath) =>
	`/__markserv/ipynb-render?path=${encodeURIComponent(relativePath)}&mode=on`;

const buildSidebarFileLink = (rootDir, absoluteFilePath) => {
	const relativePath = toPosixPath(path.relative(rootDir, absoluteFilePath));
	if (isNotebookFile(absoluteFilePath)) {
		return {
			href: buildNotebookRenderHref(relativePath),
			path: relativePath,
			fileType: "ipynb",
		};
	}
...
const dirToHtml = (filePath, rootDir, flags) => {
...
const buildNavTreeHtml = (rootDir, currentDir, flags) => {
	...
	const readDirRecursive = (dir) => {
		...
		for (const mdFile of mdFiles) {
			...
			const linkInfo = buildSidebarFileLink(rootDir, absoluteFilePath);
...
const buildRecentEntries = (rootDir, currentFilePath, limit = 10, flags) => {
	const excludeDirs = new Set(["node_modules"]);
	const entries = [];
	const absoluteRootDir = path.resolve(rootDir);
	...
	const walk = (dir) => {
		...
		for (const subPath of urls) {
			...
			const linkInfo = buildSidebarFileLink(absoluteRootDir, absolutePath);
			entries.push({
				href: linkInfo.href,
				path: linkInfo.path,
...
const createRequestHandler = (flags, runtimeState = {}) => {
	...
	return (req, res) => {
		...
		if (req.method === "GET" && decodedUrl === "/__markserv/ipynb-render") {
		...
		if (isMarkdown) {
			...
			getFile(filePath)
				...
				.then(({ output, editDataJson }) => {
					...
					const recentEntries = buildRecentEntries(dir, filePath, 10, flags);
			...
			try {
				...
				const recentEntries = buildRecentEntries(dir, null, 10, flags);

				const handlebarData = {
					dirname: path.parse(filePath).dir,
					content: dirToHtml(filePath, dir, flags),
					title: path.basename(path.resolve(dir)),
					navTree,
					recentEntries,
					rootDirStem: path.basename(path.resolve(dir)),
...
const startHTTPServer = async (connectApp, requestedPort, flags) => {
	...
	const forceReplace = flags && flags.forceReplace === true;

	if (forceReplace && !(await isPortAvailable(parsedRequestedPort))) {
		const killed = await killProcessOnPort(parsedRequestedPort);
		if (killed) {
			const freed = await waitForPortFree(parsedRequestedPort);
			if (!freed) {
				throw new Error(
					`Port ${parsedRequestedPort} could not be freed. Try stopping the process manually.`,
				);
			}
		}
	}
...
const startLiveReloadServer = (liveReloadPort, flags) => {
	...
	const markdownDirectoryIndex = getMarkdownDirectoryIndex(dir, flags);

	if (markdownDirectoryIndex) {
		const watchDirs = [...markdownDirectoryIndex.includedDirs].sort();
	...
	return liveReloadServer.watch(path.resolve(dir));
};
...
const init = async (flags) => {
	...
	const additionalServers = await Promise.all(
		addresses.slice(1).map(async (addr) => {
			...
			await listen(additionalServer, httpPort, addr);
