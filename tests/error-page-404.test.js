	...
	getPort().then(port => {
		...
		markserv.init(flags).then(service => {
			...
			request(opts, (err, res, body) => {
				...
				// Write expected:
				// fs.writeFileSync(path.join(__dirname, "error-page-404.expected.html"), body);

				const sanitize = text => text.replace(/PID:[\s\d]+</, "PID: N/A<")
					.replace(/<p class="errorMsg">(.*?)<\/p>/, "")
					.replace(/<pre>(.*?)<\/pre>/s, "")
					.replace(/<title>404: (.*?)<\/title>/, "<title>404: NORMALIZED_PATH</title>")
					.replace(/<a class="button back" href="(.*?)">Go Back<\/a>/, "<a class=\"button back\" href=\"NORMALIZED_BACK\">Go Back</a>");
